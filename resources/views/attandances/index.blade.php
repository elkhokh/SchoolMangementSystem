...
{{-- choise day  --}}
        <div class="mb-3">
            <form action="{{ route('attandances.index') }}" method="GET" class="form-inline">
                <label for="date" class="mr-2">اختار اليوم اللي هتاخد فيه الغياب</label>
                <input type="date" id="date" name="date"
                    {{-- value="{{ request('date', date('Y-m-d')) }}" --}}
            value="{{ old('date', $select_day ?? date('Y-m-d')) }}"
            max="{{ date('Y-m-d') }}"
                class="form-control mr-2">
                <button type="submit" class="btn btn-dark">اكد الاختيار</button>
                 @error('date')
...
($class->students->pluck('attendances')->flatten()->where('attendence_date', $select_day)->count())
...
{{-- mark --}}
                ...
                <div id="collapse{{ $class->id }}" class="collapse" aria-labelledby="heading{{ $class->id }}" data-parent="#accordionExample">
                                <form action="{{ route('attandances.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="attendence_date" value="{{ $select_day }}">
